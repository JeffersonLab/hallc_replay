void replay_cosmics_hms(Int_t RunNumber=0, Int_t MaxEvent=0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/hms_replay_cosmics_%d_%d.root";
  ...
  */
  ...
  analyzer->SetOdefFile("DEF-files/HMS/COSMICS/hstackana_cosmics.def");
  // Define cuts file
  ...
  analyzer->SetSummaryFile(Form("REPORT_OUTPUT/HMS/PRODUCTION/summary_cosmics_%d_%d.report", RunNumber, MaxEvent));    // optional
  // Start the actual analysis.
  analyzer->Process(run);
  // Create report file from template.
  analyzer->PrintReport("TEMPLATES/HMS/PRODUCTION/hstackana_production.template",
			Form("REPORT_OUTPUT/HMS/PRODUCTION/replay_hms_cosmics_%d_%d.report", RunNumber, MaxEvent));
