...
void test::SlaveBegin(TTree * /*tree*/)
{
   ...
   //Check option
   if (option == "readall" || option == "readall&&NGC" || option == "readall&&showall") fFullRead = kTRUE;
   if (option == "NGC" || option == "readall&&NGC" || option == "NGC&&showall") fNGC = kTRUE;
   if (option == "showall" || option == "readall&&showall" || option == "NGC&&showall") fFullShow = kTRUE;
   if (option == "lowlight") fLowLight = kTRUE;
  ...
  for (Int_t itrack = 0; itrack < Ndata_P_tr_p; itrack++) 
	...
	{	  
	  ...
	  if (fNGC ? P_tr_y[0] + P_tr_ph[0]*fngc_zpos >= 0.0 && P_tr_x[0] + P_tr_th[0]*fngc_zpos >= 0.0: 
	             P_tr_y[0] + P_tr_ph[0]*fhgc_zpos >= 4.6 && P_tr_x[0] + P_tr_th[0]*fhgc_zpos >= 9.4) fNGC ? fPulseInt_quad[0][ipmt]->Fill(P_ngcer_goodAdcPulseInt[ipmt]): 
	                                                                                                        fPulseInt_quad[0][ipmt]->Fill(P_hgcer_goodAdcPulseInt[ipmt]);
	  //Condition for quadrant 2 mirror
	  if (fNGC ? P_tr_y[0] + P_tr_ph[0]*fngc_zpos < 0.0 && P_tr_x[0] + P_tr_th[0]*fngc_zpos >= 0.0: 
	             P_tr_y[0] + P_tr_ph[0]*fhgc_zpos < 4.6 && P_tr_x[0] + P_tr_th[0]*fhgc_zpos >= 9.4) fNGC ? fPulseInt_quad[1][ipmt]->Fill(P_ngcer_goodAdcPulseInt[ipmt]): 
												               fPulseInt_quad[1][ipmt]->Fill(P_hgcer_goodAdcPulseInt[ipmt]);
	  //Condition for quadrant 3 mirror
	  if (fNGC ? P_tr_y[0] + P_tr_ph[0]*fngc_zpos >= 0.0 && P_tr_x[0] + P_tr_th[0]*fngc_zpos < 0.0: 
	             P_tr_y[0] + P_tr_ph[0]*fhgc_zpos >= 4.6 && P_tr_x[0] + P_tr_th[0]*fhgc_zpos < 9.4) fNGC ? fPulseInt_quad[2][ipmt]->Fill(P_ngcer_goodAdcPulseInt[ipmt]): 
												               fPulseInt_quad[2][ipmt]->Fill(P_hgcer_goodAdcPulseInt[ipmt]);
	  //Condition for quadrant 4 mirror
	  if (fNGC ? P_tr_y[0] + P_tr_ph[0]*fngc_zpos < 0.0 && P_tr_x[0] + P_tr_th[0]*fngc_zpos < 0.0: 
	             P_tr_y[0] + P_tr_ph[0]*fhgc_zpos < 4.6 && P_tr_x[0] + P_tr_th[0]*fhgc_zpos < 9.4) fNGC ? fPulseInt_quad[3][ipmt]->Fill(P_ngcer_goodAdcPulseInt[ipmt]): 
												              fPulseInt_quad[3][ipmt]->Fill(P_hgcer_goodAdcPulseInt[ipmt]);
	}  
    }
...
void test::Terminate()
{
  ...
  //Begin the determination of calibration constant
  ...
  for (Int_t ipmt=0; ipmt < (fNGC ? fngc_pmts : fhgc_pmts); ipmt++)
	...
	{
	  fNGC ? fPulseInt_quad[iquad][ipmt]->Rebin(10) : fPulseInt_quad[iquad][ipmt]->Rebin(10);
	}
      fNGC ? fPulseInt[ipmt]->Rebin(10) : fPulseInt[ipmt]->Rebin(10);
    }
  ...
  for (Int_t ipmt=0; ipmt < (fNGC ? fngc_pmts : fhgc_pmts); ipmt++)
      ...
      for (Int_t ibin=0; ibin<nbins; ibin++)
	{
      ...
      Poisson->SetParLimits(0, 5.2, 7.0);
  ...
  cout << "Calibration constants are (PMT#:  First Guess  Second Guess): " << endl;
  for (Int_t i=0; i<4; i++)
    {
