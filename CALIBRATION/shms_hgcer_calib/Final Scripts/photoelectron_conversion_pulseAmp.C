  ...
  for (int i=0;i<4;i++) 
    {
      for (int j=0;j<4;j++)
	{
	  hgc_e[i][j] = (TH1F*)F->Get(Form("phgc_quad%d_pulseAmp_e%d", i+1, j+1));
	  hgc_pi[i][j] = (TH1F*)F->Get(Form("phgc_quad%d_pulseAmp_pi%d", i+1, j+1));
	  hgc_sum[i][j] = (TH1F*)F->Get(Form("phgc_quad%d_pulseAmp_sum%d", i+1, j+1));
	}
    }
